...
- 新增英文单词时自动查询 IPA 音标和多个词性；查询失败不会影响词条保存。
- 使用 SQLite 在本机持久化保存数据，重启应用后仍可继续使用。

## 数据与隐私

所有词汇数据仅保存在本机的应用数据目录中，数据库文件名为 `vocabulary.db`。数据库保存词汇、IPA 音标和词性。新增英文单词时，应用会将该单词发送给 [Free Dictionary API](https://dictionaryapi.dev/) 查询 IPA 与词性；来源 URL、熟悉度和本地数据不会上传或同步。
...
如需将词汇数据保存到其他磁盘，可在应用右上角的齿轮按钮中通过系统文件夹选择器选择数据目录。保存时应用会复制并校验现有的 `vocabulary.db`，再切换到新目录；以后启动也会继续使用该位置。目标目录必须可写入，且不能已包含同名数据库文件。
